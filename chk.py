...
# ------------------------
premium_users = {}  # e.g. { "user_id": {"duration_days": X, "expiry": datetime_object} }
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    welcome_text = (
        ...
        "📂 Send a file containing only the cards and I will check them.\n\n"
        "❗ Correct format for the cards: `CC|MM|YYYY|CVV`\n\n"
...
async def upload_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    Accepts a TXT file upload containing card numbers (one per line or comma-separated)
    and stores them in user_data['cards'].
    """
...
async def chk(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    /chk command: Asynchronously checks all stored cards using Stripe and returns a summary.
    """
...
async def plan(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    /plan command: Activate a premium plan for a specified user.
    Usage: /plan <user_id> <days>
...
async def check_premium(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    /check_premium command: Check the premium status of a user.
...
async def main():
    ...
    # Ensure the timezone is set with a pytz timezone (UTC in this example)
    application = ApplicationBuilder().token(bot_token).timezone(timezone("UTC")).build()
