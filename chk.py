...
# ------------------------
premium_users = {}  # Stores premium data, e.g. { "user_id": {"duration_days": X, "expiry": datetime_object} }
...
def generate_card(bin_number: str) -> str:
    """
    Generate a single card using a BIN and Luhn check digit.
    Format: 6-digit BIN + random digits up to 15 total, then 1 check digit.
    """
...
async def check_card_async(card: str) -> str:
    ...
    Returns a string "card|STATUS" where STATUS is APPROVED, DECLINED, or RISK.
    """
...
def classify_result(result_line: str) -> str:
    """
    Extract the status from "card|STATUS".
    """
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    /start command: Sends a welcome message with inline keyboard menu.
    """
    welcome_text = (
        "🤖 **Bot status:** working ✅\n\n"
        "This bot is designed to include all the tools you need for carding.\n"
        "📂 Send a file containing only the cards and I will check it.\n\n"
        "❗ Don't forget that the correct format for the cards in the file is: `CC|MM|YYYY|CVV`\n\n"
        "⏳ You can use the bot for half an hour a day for free through the gift button below.\n"
    )
    keyboard = [
        [
            InlineKeyboardButton("🎁 Use The Daily Gift", callback_data="daily_gift"),
...
async def inline_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "daily_gift":
        ...
        if user.username and user.username.lower() == OWNER_USERNAME.lower():
            ...
            premium_users[str(user.id)] = {"duration_days": 365 * 100, "expiry": expiry}
            await query.edit_message_text("🔓 Owner Mode Activated: You are automatically premium for 100 years!")
        else:
...
async def gen(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    /gen command: Generate card numbers from a provided 6-digit BIN.
    Usage: /gen <6-digit BIN> <count (optional)>
    """
    if not context.args:
...
async def chk(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    /chk command: Asynchronously checks all stored cards and returns a summary with details.
    """
...
async def plan(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    /plan <user_id> <days> command: Activates a premium plan.
    Example: /plan 7704958976 7
    """
    args = context.args
    if len(args) != 2:
    ...
    premium_users[target_user_id] = {"duration_days": duration_days, "expiry": expiry}
    reply_text = (
...
async def check_premium(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    /check_premium <user_id> command: Checks premium status.
    """
    args = context.args
    if len(args) != 1:
